...
- current Configurator UI/UX authority is the implemented application on `origin/main` unless an exact registered Figma state has explicit `OWNER_ACCEPTED_DESIGN` status for a newer design delta;
- Figma can polish presentation but cannot silently change Product Truth, fields, allowed values, requiredness, validation, readiness, formulas, pricing, permissions, lifecycle or execution semantics;
- existing accepted surfaces use the Cursor functional implementation → runtime proof → Figma polish → Cursor design-delta → runtime parity roundtrip;
...
runtime-ul o dovedește, Figma face polish vizual, Owner/ChatGPT acceptă designul,
apoi Cursor implementează numai design delta și verifică parity.
Figma nu schimbă Product Truth pe ascuns.
