...
NEXT_STEP               = Independent review of the draft PR
THEME_IMPACT            = BOTH
...
BACK_CONTEXT_MECHANISM = discriminated session + location.state origin
```

Session keys: `workos.requests.workspaceOrigin` and `workos.requests.pendingClientHubOrigin`. Registry mount clears the bound origin. Hub entry with `?customer=` writes a pending Hub origin; a successful create binds `requestId` and navigates with that origin. Registry row clicks write `{ kind: "registry" }` and restore search/scroll. No `history.length` or `document.referrer`.
...
WEB_UNIT                     = 197 passed
API_UNIT_FIRST_RUN           = 249 passed / 2 failed (api-shutdown timeout + cloud two-process SQLITE_ERROR)
API_UNIT_ISOLATED_RERUN      = 39 passed (those two files)
BUILD                        = PASS
PLAYWRIGHT_REQUESTS_OVERVIEW = 2 passed
PLAYWRIGHT_PRECOMMIT_CLOSURE = 1 passed
PLAYWRIGHT_INSTALLATION      = 1 passed
PLAYWRIGHT_OPTIONAL_SITE     = 2 passed
PLAYWRIGHT_CLIENT_WORKSPACE  = 1 passed
FOCUSED_PLAYWRIGHT           = 7 passed / 0 failed / retries=0
```
