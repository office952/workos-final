...
Full GitHub e2e failed after the shell amendment because `completeOperatorIdentifyForm()` used page-global `Persoană` / `PIN` / `Confirmă` while Execution also has a person control in content. The helper now scopes those fields to dialog `Identifică operatorul`. The page-level Atelier form stays scoped to `form.operator-identify-form`. On `/atelier` and `/execution/*` it waits for `Ieși`, `Identifică-te`, or the page form before choosing a path, so Execution is not sent through the cookie-only API fallback while the chip is still painting. That fallback stays only for commercial pages without interactive chrome, where navigating to `/atelier` remounted the current page and dropped in-memory product confirmation. No product UI change.
