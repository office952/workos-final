...
Organization provisioning is fail-closed. A new organization starts `PROVISIONING` and becomes `ACTIVE` only after a valid Operational Plane, Cloud user, and the initial Owner membership exist. Incomplete or retryable organizations are not login-ready, including a leftover `ACTIVE` organization with no Owner. Fresh create refuses when any resume-eligible organization already exists in that root. Control Plane critical sections (organization + plane registration, then user + initial Owner + `ACTIVE`) each use one `BEGIN IMMEDIATE` SQLite transaction. The Operational Plane is a second SQLite file and is not a distributed transaction (`CONTROL_PLANE_TRANSACTIONAL_BOUNDARIES` + `DURABLE_RESUMABLE_PROVISIONING_STATE`). Resume is explicit: `cloud:provision -- --resume --root <root> --organization-id <id> --email <email>`. The first valid resume compare-and-sets `provision_owner_email`. Display-name lookup, automatic “first incomplete organization” recovery, and `--password` argv are forbidden. Later additional Owners stay a separate product decision; initial provisioning creates at most one.
