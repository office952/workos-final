...
R2 is `RESEARCH_ACCEPTED_WITH_ADVISORIES`. UI20-R3 is `CHARACTER_PROOF_ACCEPTED_WITH_ADVISORIES`. S1-A is the accepted structural base. G and IA-3 remain leading research hypotheses, not Owner-accepted directions. The next recommended step is `UI20_R4_PAGE_PERSONALITY_AND_SEMANTIC_DYNAMICS`.
