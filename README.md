...
Product catalog plus first canonical LETTERS product, with FACE / VOLUME / BACK / LIGHTING as roles, reusable constructive types, Product System inspection on `/components`, persisted display-label administration on `/admin`, Resources / Cost inspection on `/admin/resources`, Operational Processes inspection on `/admin/processes` including Letters vinyl, RAL, electrical, closure, QC and packing composition, Workcenters / Machines capability-provider inspection on `/admin/workcenters` with two owner-confirmed assembly tables and no invented machines, owner-facing governance, canonical component technical settings, and lighting calculation left partial because LED geometry, module watts, and PSU catalog identity are still missing.
