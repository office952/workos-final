...
The app has a platform shell, a real health check, a hierarchical product catalog, and a schema-driven configuration flow for the first canonical product. Product System display labels persist in local SQLite after bootstrap. Resources remain a typed catalog. Persisted execution can assign a provider, start and complete a task, and record minimal completion evidence. Technical settings, inventory, actual costing, and commercial price are not writable yet.
