...
Primary nav is Lucrări + Cereri + Oferte + Produse + Administrare. `/` is the operational job overview. `/requests` is the incoming Cerere de ofertă queue. `/quotes` is the frozen-offer registry. Inspection surfaces live under Administrare. See `docs/architecture/UI_UX_FOUNDATION_CANON.md`.
