...
Hierarchical product catalog plus two live products: LETTERS and Panou ACM casetat. Product Batch 1 discovery is `PARTIAL_NO_INVENTION`: halo-lit and full-aluminium categories stay empty until Owner construction and rate truth exist.
...
- Product cost-plus must not price operational services. Service commercial is a separate Owner-only manual-fixed channel; the pre-quote wave is local in review.
...
- OS-S1 is implemented on main. OS-S2 typed facts are implemented on main. Transaction safety is closed. OS-S3 + OS-S4 + OS-S5 are `IMPLEMENTED_LOCAL_IN_REVIEW` on `feat/first-real-letters-prequote-v1`. They are not Owner-accepted and not on main. OS-S6 transport and later slices still need a separate Owner GO. Follow-up facts remain unimplemented: ACCESS_METHOD_AND_EQUIPMENT, FIXINGS_CONSUMABLES, SITE_PHOTOS. CREW_SIZE and PLANNED_DURATION are in this local wave. V3 navigation design is Owner-accepted. Wave 1 sidebar implementation is `INTEGRATED_ON_MAIN`. Cereri V3 Figma is Owner-accepted. Cereri V3 runtime is `OWNER_ACCEPTED` and `INTEGRATED_ON_MAIN`. This file does not authorize Oferte V3, Lucrări V3, Wave 2, page-content reorganization, missing pages, Home, real Cloud write, or a live first-job quote.
