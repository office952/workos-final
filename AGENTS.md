...
LIGHTING calculation is IMPLEMENTED_CURRENT / functional V1: confirmed volume perimeter → module quantity → load → multi-PSU selection → generic EIC. `ledPitchMm`, `ledModulePowerW` and `psuReservePercent` are configurable technical settings. LETTERS Service/Labor recipes are functional V1. Canonical none/none EIC is 595.00 EUR PARTIAL because cost evidence is still in workshop calibration, not because geometry is missing. Operator-confirmed FACE area and VOLUME perimeter are valid Product Truth. Analyzer remains a future external proposal source. Values are development configuration, not final workshop calibration.
