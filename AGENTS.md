...
`/components` is the Product System inspection surface. `/admin` is the owner surface: Product System display-label write, Resources / Cost inspection, Operational Processes inspection including Letters process composition, and Workcenters / Machines capability-provider inspection. Live workcenter and machine rows are empty until identities are owner-confirmed. The first persisted write is display-label only.
