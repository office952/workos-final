...
export const REQUESTS_REGISTRY_SCROLL_PREFIX = "workos.requests.registry.scroll:";
const RESTORE_FRAME_BUDGET = 24;

export function requestsRegistryScrollStorageKey(locationKey: string): string {
  return `${REQUESTS_REGISTRY_SCROLL_PREFIX}${locationKey}`;
}
...
export function persistRequestsRegistryScroll(locationKey: string) {
  if (!locationKey) {
    return;
  }
  sessionStorage.setItem(requestsRegistryScrollStorageKey(locationKey), String(readScrollY()));
}

export function readRequestsRegistryScrollY(): number {
  return readScrollY();
}

export function useRequestsRegistryScroll(enabled: boolean) {
  const location = useLocation();
  const { key, state } = location;
  const restoringRef = useRef(false);
  const restoreY =
    state && typeof state === "object" && "restoreRequestsRegistryScroll" in state
      ? (state as { restoreRequestsRegistryScroll?: unknown }).restoreRequestsRegistryScroll
      : undefined;
  const freshVisit =
    state && typeof state === "object" && "requestsFreshVisit" in state
      ? Boolean((state as { requestsFreshVisit?: unknown }).requestsFreshVisit)
      : false;
  ...
  useLayoutEffect(() => {
    ...
    const stored = key && sessionStorage.getItem(requestsRegistryScrollStorageKey(key));
  ...
  useEffect(() => {
    ...
    const persist = () => {
      ...
      persistRequestsRegistryScroll(key);
    };
