...
export function ResolutionField() {
  ...
  return (
    <article className="ui20-surface" data-surface="cerere">
      <h1>{page.detail.request.title}</h1>
      <p className="ui20-kicker">
        {page.detail.request.reference} · Câmp de rezoluție. Ce știm, ce rămâne deschis, care
        este următoarea acțiune validă.
      </p>
      <section className="ui20-cluster" aria-labelledby="known-heading">
        <h2 id="known-heading">Ce știm</h2>
        <dl>
          {known.map((fact) => (
      ...
      <section className="ui20-cluster" aria-labelledby="unresolved-heading">
        <h2 id="unresolved-heading">Nerezolvat</h2>
        {unresolved.length === 0 ? (
          <p>Nimic deschis pe această cerere.</p>
        ) : (
          unresolved.map((item) => (
            <div key={item.id} className="ui20-item" data-energy={item.energy}>
      ...
      <section
        className="ui20-cluster"
        id="request-installation"
